...
class FuzzyImageCNN:
    ...
    #     seq = layers.MaxPooling2D(self.pooling)(seq)
    #
    #     for i in np.arange(self.conv_layers-1):
    #         seq = layers.Conv2D(self.filters * (i+2), self.kernel_size, padding="same", activation='relu', input_shape=(self.image_dim, self.image_dim, 1))(seq)
    #         seq = layers.MaxPooling2D(self.pooling)(seq)
    #
    #     output = Flatten()(seq)
    #
    ...
    # def design_network(self):
    #     print(" Pre-built Keras Inception")
    #
    #
    #     from keras import layers
    #     from keras.applications.inception_v3 import InceptionV3
    #
    #     base_model = InceptionV3(weights=None, include_top=False, input_shape=(self.image_dim, self.image_dim, 1))
    #
    #     x = base_model.output
    #
    #     seq = layers.MaxPooling2D(self.pooling)(x)
    #
    #     output = Flatten()(seq)
    #
    #     for i in np.arange(self.dense_layers):
    ...
    #     out = Dense(1, activation='linear')(output)
    #
    #     self.model = Model(input=base_model.input, outputs=out)
    #     self.model.compile(loss='mse', optimizer='adam')
